...
elseif($model=="hejin"){
	if(GetIP()==lang('plugin/hejin_box', 'fstp')){
		if($_GET['vid']){
			$vid=intval($_GET['vid']);
			//$vote =  C::t('#hejin_box#hjtp_votes')->fetch_by_id($vid);
			if($_GET['caozuo']=='info'){
				echo '<pre>';
				print_r($vote);
				echo '</pre>';
			}elseif($_GET['caozuo']=='over'){
				$votetpup = array();
				$votetpup['is_sh']=intval($vote['is_sh']+2);
				//$upvotetp =  C::t('#hejin_box#hjtp_votes')->update_by_id(intval($vote['id']),$votetpup);
			}elseif($_GET['caozuo']=='sha'){
