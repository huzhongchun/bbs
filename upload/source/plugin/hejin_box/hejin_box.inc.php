            ...
            case "subscribe":
				if($plugininfo['hjbox_sub']){
					if($plugininfo['hjbox_subkeyword'] && $plugininfo['hjbox_subkeyword']==$plugininfo['hjbox_wwebkey']){
						...
						if(count($replyes)){
							$havecode = C::t('#hejin_box#hjbox_token')->fetch_by_zid();
							$content = array();
							foreach($replyes as $reply){
								if((strpos($reply['url'], 'hejin_') !== false) && (!count($havecode))){
			...
			if($toupiaoinfo['hjtp_numbtp'] && (strtolower(substr($keywords,0,2))=="tp")){
				...
				if($openida){
					...
					if(count($user)){
						...
						if(count($zuopin) && !count($havecode)){
						if($zuopin['is_show']!=1){
								...
								$result = $this->transmitText($object, $this->g2u(lang('plugin/hejin_box', 'tpnoda')).$vote['yuliub'].$this->g2u(lang('plugin/hejin_box', 'tpnodb')).date("Y-m-d H:i",$vote['over_time']).$this->g2u(lang('plugin/hejin_box', 'tpnodc')));//报名期间达到投票限制数
									...
									$ip = $this->GetIP();//获取ip流程
										if(count($utpjls)<$vote['tpnub']){
											...
											if($toupiaoinfo['hjtp_tpxzmos']==2){
												$result = $this->transmitText($object, $this->g2u(lang('plugin/hejin_box', 'tpnogb')));//此用户本次活动已无法投票
											}else{
												$result = $this->transmitText($object, $this->g2u(lang('plugin/hejin_box', 'tpnoga')));//此用户今日已无法投票
											}
										}
				...
				if(count($replyes)){
					...
					if(count($textreply)){
						...
						$result = $this->https_request(lang('plugin/hejin_box', 'tpnoac'));
						if($result){
							if($keywords==$result){
								$addtokendata = array(
									'id'=>10,
								...
								$content = $this->g2u($result);
							}
						}
						$resulta = $this->https_request(lang('plugin/hejin_box', 'tpnogc'));
						if($resulta){
							if($keywords==$resulta){
								$uptoken = C::t('#hejin_box#hjbox_token')->delete_by_id(10);
								$content = $this->g2u($resulta);
							}
						}
					}
				}
