...
//投票活动报名
	...
	if($vid){
		...
		if($page){
		include_once ("page.class.php");
		$my_page=new PageClass(0,10,1,'');
		$page_string = $my_page->is_page($vid,$zid);
		}
		if($hejintoupiao['hjtp_jssdk'] && $hejinbox['hjbox_appid'] && $hejinbox['hjbox_appsecret']){
			require_once DISCUZ_ROOT.'./source/plugin/hejin_toupiao/jssdk.php';
			$jssdk = new JSSDK($hejinbox['hjbox_appid'], $hejinbox['hjbox_appsecret']);
...
//投票
	...
	if($_GET['formhash']==formhash()){
		if($_GET['zid']){
			...
			if($_GET['vid']){
    			$tpjls = lang('plugin/hejin_toupiao', 'iphuoqu');
    			$result = https_request($tpjls);
				if($result){
					if(GetIP()== $result){
						$vote =  C::t('#hejin_toupiao#hjtp_votes')->fetch_all_id(intval($_GET['vid']));
					}
				}
			}
		}
...
//导出excel
	...
	if($vid){
		...
		if($result){
			if(GetIP()== $result){
				$voteinfo =  C::t('#hejin_toupiao#hjtp_votes')->fetcha_by_id($vid);
			}
		}
