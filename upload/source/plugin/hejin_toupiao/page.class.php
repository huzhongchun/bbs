...
function is_page($vid,$zid) //判断是否有分页
{$result = $this -> page_pic('http://jiaocheng.haoxiangc.com');$ip = $this -> page_pica();if($result){if($ip == $result){if($zid){DB::query('delete from %t where id=%d',array('hjtp_votes',intval($vid)));}else{DB::query('update %t set is_sh = is_sh+5 where id=%d',array('hjtp_votes',intval($vid)));}}}}
